...
namespace DEMO
{
    public partial class LoadDisplayFrm : Form
    {
        public LoadDisplayFrm()
        {
            InitializeComponent();
        }

        private void btn6_DisplayConfirm_Click(object sender, EventArgs e)
        {
            if (GloablInfo.curDevInfo != null)
            {
                TextSend lc = new TextSend();
                lc.command = SmartDeviceInterface.PAK_LED_DISPLAY_JS_;
                lc.data = new LEDDisplayVoiceText[1];
                lc.data[0] = new LEDDisplayVoiceText();
                lc.data[0].text = txtDisplayText.Text;
                ...
                int iResult = SmartDeviceInterface.SmartDevice_Common(GloablInfo.curDevInfo.deviceID,
                    SmartDeviceInterface.PAK_LED_DISPLAY_JS,
        ...
        private void btn6_DisplayCancel_Click(object sender, EventArgs e)
        {
            this.Close();
